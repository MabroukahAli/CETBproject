# RLS Based Adaptive Beamforming for Wireless Receiver Enhancement

This repository contains the MATLAB code for a graduation project on adaptive beamforming for a QPSK wireless receiver. The project studies how the Recursive Least Squares algorithm can improve receiver performance under co-channel interference and noise. LMS and NLMS are also included for comparison.

## Project Information

Student: Mabroukah Ali Mohammed Abduljalil  
College: College of Electronic Technology Bani Walid  
Department: Communications and Microwaves  
Supervisor: Dr. Ismail AL-Marimi  
Year: 2026

## Repository Contents

- `Code/MAINCODE.m`: Main MATLAB script used to generate the simulation results.

## How to Run

1. Open MATLAB.
2. Open the `Code` folder.
3. Run `MAINCODE.m`.
4. The script generates the figures and tables used for the project results.

## Main Topics

- QPSK wireless receiver simulation.
- Uniform linear antenna array.
- RLS adaptive beamforming.
- LMS and NLMS comparison.
- BER, SINR, MSE, EVM, radiation pattern, and array size analysis.
