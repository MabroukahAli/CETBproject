...
%  Adaptive Antenna Array Beamforming using the Recursive Least Squares
%  (RLS) Algorithm for Interference Suppression in a QPSK Wireless Link
% =========================================================================
%  This script is self-contained and reproduces every figure and table
%  required for the "Results" chapter of the project. Running it from
%  start to finish (F5) regenerates the complete set of figures (saved as
%  PNG) and tables (saved as PNG + CSV/XLSX) used in the chapter.
%
%  Adaptive technique: Recursive Least Squares (RLS) is the core adaptive
%  algorithm implemented here. In addition, Least Mean Squares (LMS) and 
%  Normalized LMS (NLMS) are implemented for performance and convergence 
%  speed comparison, as outlined in the presentation's future work.
%
%  Reference basis for the reported metrics:
%  - RLS learning curves and forgetting-factor trade-off follow standard
%    adaptive-filter analysis.
%  - BER curves use the standard coherent QPSK/BPSK AWGN and Rayleigh
%    reference expressions.
%  - Array figures report main-beam preservation, null depth, HPBW, and
%    output SINR, which are standard adaptive beamforming performance
%    indicators for a ULA.
%
%  OUTPUT FILE NAMING CONVENTION (as required for the thesis chapter):
%       "4" (chapter number, fixed) + sequential figure index
%       e.g. the 1st figure  -> 41.png
%            the 15th figure -> 415.png
%  Tables are exported as Table_*.png + CSV + XLSX.
% =========================================================================
...
%  --------------------------------------------------------------------
CFG.ChapterNumber   = 4;                       % Fixed chapter prefix
CFG.OutDir          = fullfile(pwd,'Chapter4_Figures');
if ~exist(CFG.OutDir,'dir'); mkdir(CFG.OutDir); end

% --- Physical / signal model (kept identical to the baseline model) ----
CFG.NumofAntenna    = 10;                      % Baseline ULA size
CFG.Kd              = pi;                      % Element spacing = lambda/2
CFG.ModuRadians     = pi/4;                    % QPSK (Gray-coded)
CFG.theta_tx        = deg2rad(60);             % Desired user DoA
CFG.theta_Int1      = deg2rad(30);             % Interferer #1 DoA
CFG.theta_Int2      = deg2rad(120);            % Interferer #2 DoA
CFG.SIR1_dB         = 3;                       % Baseline SIR, interferer 1
CFG.SIR2_dB         = 3;                       % Baseline SIR, interferer 2

% --- RLS algorithm parameters (baseline, matches original design) ------
CFG.lambda          = 0.75;                    % Forgetting factor
CFG.delta           = 1e-2;                    % Regularization for P(0)

% --- SNR sweep used for BER curves --------------------------------------
CFG.EbNo_dB         = -12:2:15;
CFG.SNR_dB          = CFG.EbNo_dB + 10*log10(2);
CFG.SNR_demo_dB     = 12;                      % Working point for the
                                                % convergence / pattern /
                                                % constellation figures

% --- Monte-Carlo budget (INCREASE for the final thesis-quality run) ----
% NOTE FOR THE STUDENT:
%   These default values complete the ENTIRE chapter (16 figures + 6
%   tables) in a few minutes, which is convenient while you are still
%   arranging figures/captions in the project document. Once the chapter
%   layout is final, increase BitsPerBlock / MinErrors / MaxBlocks (for
%   example to 1e6 / 100 / 200) and re-run the script once overnight to
%   regenerate every figure/table with publication-grade statistical
%   confidence before submitting the thesis.
CFG.BitsPerBlock    = 20000;    % bits generated per Monte-Carlo block
CFG.MinErrors       = 20;       % stop a BER point after this many errors
CFG.MaxBlocks       = 15;       % hard cap on blocks per BER point

CFG.figIdx = 0;   % running figure counter (do not edit)
...
elementsX = (0:Nshow-1)*0.5;                  % element spacing lambda/2
origin = mean(elementsX);
arrowLen = 3.0;

% Draw the three DoA rays as plain lines with an arrowhead marker at the
% tip (portable across MATLAB/Octave versions - avoids quiver auto-scale
% quirks).
...
% Antenna elements drawn last so they sit on top of the rays
...
%     Used for figures 4.2 - 4.7 (constellation, convergence, SINR)
%  --------------------------------------------------------------------
rng(1);   % reproducibility of the illustrative run
...
y_ref = Rx_Sig(1,:);   % single-element reference (no beamforming)
...
xlim([1 NumOfSamples]);
title('RLS Learning Curve from the Prediction Error Signal');
...
CFG = save_thesis_figure(fig, CFG, ...
    'RLS prediction error shown as instantaneous and smoothed MSE, replacing the weak raw error trace with a standard learning-curve metric.');
...
%     Used for figures 4.10 and 4.11
%  --------------------------------------------------------------------
...
for kk = 1:length(N_list)
    ...
    if Nk == CFG.NumofAntenna
        RPk_dB_ref = RPk_dB; %#ok<NASGU>
    end
    RP_all{kk} = RPk_dB; %#ok<AGROW>
end
...
for kk = 1:length(lambda_list)
    ...
    thresh = SteadyMSE_dB(kk) + 1;   % within 1 dB of steady-state
...
%% ------------------------------------------------------------------
%  16. FIGURE 4.15 - BER COMPARISON: LMS vs NLMS vs RLS
%  --------------------------------------------------------------------
fprintf('\n--- Running BER comparison over SNR (LMS vs NLMS vs RLS, Fig. 4.15) ---\n');
mu_LMS = 0.002;
mu_NLMS = 0.1;
BER_lms  = nan(1,nSNR);
BER_nlms = nan(1,nSNR);
...
rng(5);
for si = 1:nSNR
    [BER_lms(si), BER_nlms(si), BER_rls(si)] = ber_algorithms_monte_carlo( ...
        CFG.NumofAntenna, CFG.Kd, CFG.theta_tx, CFG.theta_Int1, CFG.theta_Int2, ...
        CFG.SIR1_dB, CFG.SIR2_dB, CFG.SNR_dB(si), mu_LMS, mu_NLMS, CFG.lambda, CFG.delta, ...
        CFG.ModuRadians, CFG.BitsPerBlock, CFG.MinErrors, CFG.MaxBlocks);
    fprintf('  SNR = %6.2f dB | LMS = %.3e | NLMS = %.3e | RLS = %.3e\n', ...
        CFG.SNR_dB(si), BER_lms(si), BER_nlms(si), BER_rls(si));
end

fig = figure('Name','BER Comparison: LMS vs NLMS vs RLS');
semilogy(CFG.SNR_dB, BER_lms,  'o-',  'Color',[0.85 0.33 0.1], 'LineWidth',1.5); hold on; grid on; box on;
semilogy(CFG.SNR_dB, BER_nlms, 's--', 'Color',[0.49 0.18 0.56], 'LineWidth',1.5);
semilogy(CFG.SNR_dB, BER_rls,  '^-',  'Color',[0 0.45 0.74], 'LineWidth',1.7);
xlim([CFG.SNR_dB(1) CFG.SNR_dB(end)]); ylim([1e-6 1]);
title('BER Comparison of LMS, NLMS, and RLS Beamforming');
xlabel('SNR (dB)'); ylabel('Bit Error Rate');
legend('LMS beamforming', 'NLMS beamforming', 'RLS beamforming', 'Location','southwest');
CFG = save_thesis_figure(fig, CFG, ...
    'BER comparison of LMS, NLMS, and RLS adaptive beamforming across the same SNR range and interference setting.');
...
fprintf('\n=== All 16 figures generated (41.png ... 416.png) ===\n');

%% =======================================================================
%  TABLES - rendered as PNG images (continuing the figure numbering) and
%  exported as CSV / XLSX files ready to be pasted into the thesis text.
% =========================================================================
...
T1_rows = {
    ...
    'Interferer 1 DoA / SIR_1',            sprintf('%d deg / %d dB', round(rad2deg(CFG.theta_Int1)), round(CFG.SIR1_dB));
    'Interferer 2 DoA / SIR_2',            sprintf('%d deg / %d dB', round(rad2deg(CFG.theta_Int2)), round(CFG.SIR2_dB));
    'RLS forgetting factor, \lambda',      num2str(CFG.lambda);
    'RLS regularization, \delta',          num2str(CFG.delta);
    'SNR sweep range',                     sprintf('%d to %d dB', round(CFG.SNR_dB(1)), round(CFG.SNR_dB(end)));
    };
...
T2_rows = cell(nSNR,4);
for si = 1:nSNR
    T2_rows{si,1} = sprintf('%d', round(CFG.SNR_dB(si)));
    T2_rows{si,2} = format_ber_value(BER_bf(si));
    T2_rows{si,3} = format_ber_value(BER_nobf(si));
    improvementValue = (BER_nobf(si)+eps)/(BER_bf(si)+eps);
    if improvementValue >= 100
        T2_rows{si,4} = sprintf('%.0f x', improvementValue);
    else
        T2_rows{si,4} = sprintf('%.1f x', improvementValue);
    end
end
...
%% ------------------------------------------------------------------
%  TABLE 4.5 - BER COMPARISON OF LMS, NLMS, AND RLS
%  --------------------------------------------------------------------
berTableSNR = [-1 5 11];
berTableIdx = zeros(size(berTableSNR));
for kk = 1:length(berTableSNR)
    [~, berTableIdx(kk)] = min(abs(CFG.SNR_dB - berTableSNR(kk)));
end
T5_headers = {'SNR (dB)','BER - LMS','BER - NLMS','BER - RLS'};
T5_rows = cell(length(berTableIdx),4);
for kk = 1:length(berTableIdx)
    si = berTableIdx(kk);
    T5_rows{kk,1} = sprintf('%d', round(CFG.SNR_dB(si)));
    T5_rows{kk,2} = format_ber_value(BER_lms(si));
    T5_rows{kk,3} = format_ber_value(BER_nlms(si));
    T5_rows{kk,4} = format_ber_value(BER_rls(si));
end
T5 = cell2table(T5_rows, 'VariableNames', {'SNR_dB','BER_LMS','BER_NLMS','BER_RLS'});
CFG = save_thesis_table(T5, T5_headers, T5_rows, CFG, ...
    'Table 4.5', 'BER Algorithms Comparison', ...
    'Representative BER values for LMS, NLMS, and RLS adaptive beamforming at selected SNR values.');

%% ------------------------------------------------------------------
%  TABLE 4.6 - ALGORITHM CONVERGENCE COMPARISON SUMMARY
%  --------------------------------------------------------------------
T6_headers = {'Algorithm','Steady-State MSE (dB)','Convergence Samples'};
T5_rows = {
...
T6 = cell2table(T5_rows, 'VariableNames', {'Algorithm','SteadyStateMSE_dB','ConvergenceSamples'});
CFG = save_thesis_table(T6, T6_headers, T5_rows, CFG, ...
    'Table 4.6', 'Adaptive Algorithms Comparison', ...
    'Comparison of convergence speed and steady-state MSE for LMS, NLMS, and RLS.');

fprintf('\n=== All 6 tables generated (as PNG + CSV + XLSX) ===\n');
...
% Gray-coded QPSK modulator (unit-energy symbols before power scaling)
...
% Gray-coded QPSK demodulator matching qpsk_gray_modulate
B4 = (real(y) < 0);
B3 = (imag(y) < 0);
...
% Array propagation (steering) vector for a ULA
v = exp(1i*kd*(0:N-1)'*cos(theta));
end

function sinr_dB = output_sinr_db(w, a_tx, a_i1, a_i2, Pint1, Pint2, N0)
% Computes output SINR from the current beamformer weights and steering vectors.
Sdes = abs(w.'*a_tx)^2;
...
% Builds one Monte-Carlo block: desired user + 2 interferers + AWGN,
% received by an N-element ULA.
...
% Recursive Least Squares (RLS) adaptive beamformer.
w = zeros(N,1);
P = (1/delta) * eye(N);
L = size(Rx_Sig,2);
y = zeros(1,L);
...
% Least Mean Squares (LMS) adaptive beamformer.
w = zeros(N,1);
L = size(Rx_Sig,2);
y = zeros(1,L);
e = zeros(1,L);
...
% Normalized Least Mean Squares (NLMS) adaptive beamformer.
w = zeros(N,1);
L = size(Rx_Sig,2);
y = zeros(1,L);
e = zeros(1,L);
...
% Simple centred moving-average smoother (no toolbox dependency,
% replaces the Curve Fitting Toolbox "smooth" function).
x = x(:).';
n = length(x);
y = zeros(1,n);
half = floor(span/2);
...
% Toolbox-free empirical CDF for scalar performance metrics.
xs = sort(x(:).');
n = numel(xs);
if n == 0
    ps = [];
...
% Quantile helper without a Statistics Toolbox dependency.
x = sort(x(:));
if isempty(x)
    p = NaN;
    return;
...
% Estimates half-power beamwidth around the selected main-lobe peak.
...
% Monte-Carlo BER estimation for the RLS-beamformed link (BER_bf) and for
% a single reference element with no beamforming (BER_nobf).
...
if T_err_bf == 0
    BER_bf = 0.5/T_bits;     % conservative upper-bound estimate (no errors observed)
else
...
function [BER_lms, BER_nlms, BER_rls] = ber_algorithms_monte_carlo( ...
    N, kd, theta_tx, theta_i1, theta_i2, SIR1_dB, SIR2_dB, SNR_dB, ...
    mu_LMS, mu_NLMS, lambda, delta, moduRad, bitsPerBlock, minErrors, maxBlocks)
% Monte-Carlo BER estimation for LMS, NLMS, and RLS using the same received
% signal blocks. This keeps the BER comparison fair across the algorithms.

T_err_lms = 0; T_err_nlms = 0; T_err_rls = 0; T_bits = 0; blocks = 0;

while (T_err_rls < minErrors) && (blocks < maxBlocks)
    blocks = blocks + 1;
    [Rx_Sig, QPSK_TxSig, uncoded_bits_Tx] = generate_scenario( ...
        N, kd, theta_tx, theta_i1, theta_i2, SIR1_dB, SIR2_dB, SNR_dB, bitsPerBlock, moduRad);

    [y_lms, ~, ~, ~] = lms_beamformer(Rx_Sig, QPSK_TxSig, N, mu_LMS);
    [y_nlms, ~, ~, ~] = nlms_beamformer(Rx_Sig, QPSK_TxSig, N, mu_NLMS);
    [y_rls, ~, ~, ~] = rls_beamformer(Rx_Sig, QPSK_TxSig, N, lambda, delta);

    T_err_lms = T_err_lms + sum(abs(uncoded_bits_Tx - qpsk_demod(y_lms)));
    T_err_nlms = T_err_nlms + sum(abs(uncoded_bits_Tx - qpsk_demod(y_nlms)));
    T_err_rls = T_err_rls + sum(abs(uncoded_bits_Tx - qpsk_demod(y_rls)));
    T_bits = T_bits + length(uncoded_bits_Tx);
end

BER_lms = ber_from_errors(T_err_lms, T_bits);
BER_nlms = ber_from_errors(T_err_nlms, T_bits);
BER_rls = ber_from_errors(T_err_rls, T_bits);
end

function BER = ber_from_errors(errorCount, bitCount)
% Conservative BER estimate when no bit errors are observed.
if errorCount == 0
    BER = 0.5/bitCount;
else
...
% Portable polar radiation-pattern plot (0-180 deg) drawn with basic
% graphics primitives so it renders identically across MATLAB versions.
...
% Draws a simple direction-of-arrival ray from (originX,0) with an
% arrowhead marker at the tip. Portable substitute for quiver().
...
% sgtitle() was introduced in R2018b; fall back to an annotation-based
% super-title for older MATLAB releases (and for Octave testing).
try
    sgtitle(str);
catch
...
% Saves the current figure using the required naming convention:
%   ChapterNumber concatenated with the running figure index, e.g. 415.png
...
% Renders a table as a professional PNG image and additionally exports it
% as CSV and XLSX so it can be pasted directly as an editable table.
...
outfile = fullfile(CFG.OutDir, [baseName '.png']);
set(fig, 'Color', 'w');
try
    exportgraphics(fig, outfile, 'Resolution', 300);
catch
...
function s = format_ber_value(value)
s = sprintf('%.3e', value);
s = regexprep(s, '(\.\d*?)0+e', '$1e');
s = regexprep(s, '\.e', 'e');
end

function fig = draw_table_figure(titleStr, headers, dataRows)
% Draws a clean, publication-style table using basic graphics primitives
% (no uitable dependency), so it exports reliably as a PNG image.
